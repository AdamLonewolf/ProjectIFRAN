...
const addUser = () => {
    (user.name = document.getElementById("name").value),
      ...
      (user.register = document.getElementById("register").value),
    ...
    auth
      ...
      .then((res) => {
        let imgFile = document.getElementById("profilepic").files[0];
        let ext = imgFile.name.split(".").pop();
        userProfilePic = user.matricule + "." + ext;
        let ref = storageRef.child(userProfilePic);
        ref.put(imgFile);
      })
      .then(() => {
        db.collection("users")
